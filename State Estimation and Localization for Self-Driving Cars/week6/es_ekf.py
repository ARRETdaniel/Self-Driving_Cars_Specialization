...
# a function for it.
################################################################################################
def measurement_update_orginal(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
    ...
    S_k = h_jac @ p_cov_check @ h_jac.T + R
    K_k = p_cov_check @ h_jac.T @ np.linalg.inv(S_k)
    ...
    delta_theta = error_state[6:]
    delta_q = Quaternion(axis_angle=delta_theta)
    q_hat = Quaternion(*q_check).quat_mult_left(delta_q)
...
for k in range(1, imu_f.data.shape[0]):
    delta_t = (imu_f.t[k] - imu_f.t[k - 1]) / 1000  # convert ms to seconds

    # 1. Update state with IMU inputs (propagation)
    C_ns = Quaternion(*q_est[k-1]).to_mat()  # rotation matrix from quaternion
    p_check = p_est[k-1] + v_est[k-1] * delta_t + 0.5 * (C_ns @ imu_f.data[k-1] + g) * delta_t**2
    v_check = v_est[k-1] + (C_ns @ imu_f.data[k-1] + g) * delta_t
    ...
    Q_k = np.eye(6)
    Q_k[:3, :3] *= var_imu_f * delta_t**2
    Q_k[3:, 3:] *= var_imu_w * delta_t**2
