...
def measurement_update(sensor_var, p_cov_check, y_k, p_check, v_check, q_check):
   ...
   #origional k_gain = p_cov_check @ h_jac.T @ \ np.linalg.inv(h_jac @ p_cov_check @ h_jac.T + r_cov)
    k_gain = p_cov_check.dot(h_jac.T).dot(np.linalg.inv(h_jac.dot(p_cov_check).dot(h_jac.T) + r_cov))

    # 3.2 Compute error state
   #origional x_error = k_gain @ (y_k - p_check)
...
for k in range(1, imu_f.data.shape[0]):  # start at 1 b/c we have initial prediction from gt
    ...
    #new code
    ##q_cov_km = delta_t**2*np.diag([var_imu_f,var_imu_f,var_imu_f,var_imu_w,var_imu_w,var_imu_w])         # 输入（IMU）的协方差矩阵
    ...
    p_cov_check = f_jac_km.dot(p_cov[k - 1]).dot(f_jac_km.T) + l_jac.dot(q_cov_km).dot(l_jac.T)
   # print(p_cov_check)
